...
class gameObject: public DrawableObj
{
    public:
...
//
    ...
    GrManager objects;
    objects.registrate(&background);
    objects.registrate(&player);
    objects.registrate(&hud);
    objects.registrate(&Plates);
    ...
    while (window.isOpen())
    {
        ...
        Plates.workOnCollisionWithPlayer(&player ,&BRS ,&PERESD ,&hud, &objects);
        Plates.deletePlatesOutOfScreen(&objects);
        ...
        if(plateDelay == 0)
        {
            Platform* pl = generateNewPlate(PlateTextures);
            Plates.addPlate(pl);
            plateCounter += 1;
            plateDelay = 50;
            time++;
        ...
        //(*(DrawableObj*) objects.list[3]).draw(&window);
        //window.clear();
